...
async def _(event):
    ...
    await event.edit("𝗩𝗜𝗥𝗨𝗦 - جاري الفحص....")
    end = datetime.datetime.now()
    ms = (end - start).microseconds / 1000
    await event.edit(f'''
**𝗩𝗜𝗥𝗨𝗦 - تشــكــيــر فــايــروس
☆ VERSION : 3.3
☆ PING : `{ms}`
☆ DATE : `{m9zpi}`
☆ ID : `{event.sender_id}`
☆ ALSiD : @VV_RSI**
☆ Developer : @VIPTRT ☂ @en_jq
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك  @VV_RSI')
...
async def join_channel():
    try:
        await Tepthon(JoinChannelRequest("@VV_RSI"))
    except BaseException:
        pass
...
async def _(event):
      await event.reply(""" 𝗩𝗜𝗥𝗨𝗦 - تشــكــيــر فــايــروس يعمل بنجاح 
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
...
async def _(event):
      await event.reply("""فايروس : @VIPTRT ☂ @en_jq"""
)
